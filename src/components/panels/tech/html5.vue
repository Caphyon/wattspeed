<template>
  <container>
    <template v-if="!error" slot="header">
      <h3 v-if="isValid" class="html5-status text--success text--center text--strong mb0">No html issues found.</h3>
      <h3 v-else class="html5-status text--danger text--center text--strong mb0">Html issues found while checking the page.</h3>
    </template>
    <template>
      <div v-if="!isValid">
        <ul class="list-unstyled">
          <li class="scrollable--issue" v-for="(message, index) in filteredData" :key="index" :class="message.class">
            <h4 class="mt0 mb0">{{ message.msg }}</h4>
            <p v-if="message.mark" class="code mb0">
              <code>{{ message.mark }}</code>
            </p>
          </li>
        </ul>
      </div>
  ...
  methods: {
    computeInitialData(data) {
      return data.messages.map(crtItem => {
        let newItem = {};
        newItem.msg = marked.parseInline(crtItem.message);
