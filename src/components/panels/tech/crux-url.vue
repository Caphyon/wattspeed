<template>
  <pair-container class="lh-vars">
    <div class="lh-audit-group--metrics"
         :class="data.url ? 'lh-audit-group mb0' : ''">
      <div class="mb2">
        <div class="flex pt-2">
          <div>
            <img
              :alt="`Core web vitals ${getCWVStatus('url')}`"
              :src="getCWVStatusSvg('url')"
              height="60"
              class="mx-auto block"
              style="max-height: 60px;"/>
          </div>
          <p v-if="data.url" class="my-auto">
            <span v-html="getGroupText('url')"/>
          </p>
          <p v-else class="my-auto">
            <span class="lh-audit-group__title">Field Data</span>
            <span v-html="getNoDataText('url')"/>
          </p>
        </div>
      </div>
      <div class="lh-metrics-container">
        <template v-for="(metricId, index) in Object.keys(sections)">
          <div
            v-if="data.url
                && data.url.record
                && data.url.record.metrics[sections[metricId].id]"
            class="lh-metric lh-metric--numeric border-0"
            :class="`lh-metric--${getStatusClass('url', sections[metricId].id)}`"
            :key="index">
            <div class="lh-metric__innerwrap_old gap-y-1">
              ...
              <div class="lh-metric__value_old">
                {{ getPercentile('url', sections[metricId].id) }}
              </div>
              <div class="progress crux_progress col-span-2 col-start-2 h-auto">
                <div
                  class="progress-bar crux_progress--fast font-bold rounded-lg"
                  :class="{ '-mr-3 pr-4' : fastHasNext('url', sections[metricId].id) }"
                  role="progressbar"
                  v-if="getData('url', sections[metricId].id, 0, true)"
                  :style="'flex-grow:' + getData('url', sections[metricId].id, 0)"
                  :aria-valuenow="getData('url', sections[metricId].id, 0)"
                  aria-valuemin="0"
                  aria-valuemax="100">
                  {{ getData('url', sections[metricId].id, 0, true) }}%
                </div>
                <div
                  class="progress-bar crux_progress--avg font-bold rounded-lg"
                  :class="{ '-mr-3 pr-4' : avgHasNext('url', sections[metricId].id) }"
                  role="progressbar"
                  v-if="getData('url', sections[metricId].id, 1, true)"
                  :style="'flex-grow:' + getData('url', sections[metricId].id, 1)"
                  :aria-valuenow="getData('url', sections[metricId].id, 1)"
                  aria-valuemin="0"
                  aria-valuemax="100">
                  {{ getData('url', sections[metricId].id, 1, true) }}%
                </div>
                <div
                  class="progress-bar crux_progress--slow rounded-lg font-bold"
                  role="progressbar"
                  v-if="getData('url', sections[metricId].id, 2, true)"
                  :style="'flex-grow:' + getData('url', sections[metricId].id, 2)"
                  :aria-valuenow="getData('url', sections[metricId].id, 2)"
                  aria-valuemin="0"
                  aria-valuemax="100">
                  {{ getData('url', sections[metricId].id, 2, true) }}%
                </div>
              </div>
            </div>
          </div>
