...
export default {
  ...
  methods: {
    getGrade(grade) {
      return this.hasGrade(grade) ? grade.grade : "NA";
    },
    getGradeClass(grade) {
      const colour = this.hasGrade(grade) ? grade.colour : "default";
      switch (colour) {
        case "green":
    ...
    hasGrade(grade) {
      return grade != null && Object.keys(grade).indexOf("grade") >= 0
        && Object.keys(grade).indexOf("colour") >= 0;
    }
  }
};
</script>
